...
while (1)
      {
          if ( (total_overflow_count > _1msec) && (newKeyCheck == SET))
          {
            total_overflow_count = 0;
            min5_count++;
            min10_count++;
            BIT_TOG( STATUS_PORT, STATUS_LED);
            FAN1_ON();  
            min10_completed = CLR;
          }   
          
          if ( newKeyCheck == SET && min10_count > MIN10 && min10_completed == CLR)
          { 
            FAN1_OFF(); 
            FAN2_OFF();  
          ...
          if ( newKeyCheck == SET && min5_count > MIN5 && min5_completed == CLR)
          {     
            min5_count = 0;
            FAN2_ON(); 
            min5_completed = SET;
